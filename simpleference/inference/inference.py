...
def run_inference_n5_multi(prediction,
    ...
    io_outs = []
    for sf, target_key, channel_order in zip(save_file, target_keys, channel_orders):
        io_outs.append(IoN5(sf, target_key, channel_order))
...
def run_inference_n5(prediction,
    ...
    io_outs = []
    for target_key in zip(target_keys, channel_orders):
        io_outs.append(IoN5(save_file,target_key, channel_order))
...
def run_inference(prediction,
    ...
    def write_output(output, output_bounding_box):
        for io_out, out in zip(io_outs, output):
            print("Write output of shape", out.shape ,"to", output_bounding_box)
            io_out.write(out, output_bounding_box)
        return 1

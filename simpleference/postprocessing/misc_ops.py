...
def threshold_cc(input_, offsets_wc, thr=0., ds_shape=(1, 1, 1), output_shape_wc=(1, 1, 1)):
    if not isinstance(input_, np.ndarray):
        input_ = np.array(input_)
    output = (input_ > thr).astype(np.uint64)
    scipy.ndimage.label(output, output=output)
    block_offset = [int(offsets_wc[k] // output_shape_wc[k]) for k in range(len(output_shape_wc))]
